...
def details(request,id=0):
    if(request.method=="GET"):
        if(id==0):
            ...
            form_1=other(instance=id_value_1)
        return render(request,'details.html',{"form":form,"form_1":form_1})
    else:
        if (id==0):
            form=employee(request.POST)
            form_1=other(request.POST)
        else:
            id_value=employee_details.objects.get(id=id)
            id_value_1=other_details.objects.get(id=id)
            form=employee(request.POST,instance=id_value)
            form_1=other(request.POST,instance=id_value_1)
        print(form.is_valid(),form_1.is_valid())
